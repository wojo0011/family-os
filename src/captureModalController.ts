...
const captures: CaptureDefinition[] = [
  ...
  { kind: 'Home maintenance', icon: '⌂', description: 'Maintenance task, due date, status and cadence', accent: '#dec66e' },
  { kind: 'Safety record', icon: '🧯', description: 'Home safety item, inspection status and next check', accent: '#ff9b78' },
  { kind: 'Appliance', icon: '🔌', description: 'Model, serial, warranty and appliance details', accent: '#7cc7ff' },
...
const schemas: Record<CaptureKind, FieldDefinition[]> = {
  ...
  'Home maintenance': [
    ...
    { label: 'Area', name: 'area', type: 'select', options: [...HOME_AREAS] },
    { label: 'Due / completed date', name: 'date', type: 'date', required: true, defaultValue: 'today' },
    { label: 'Repeat', name: 'repeat', type: 'select', options: [...HOME_MAINTENANCE_REPEATS] },
    { label: 'Status', name: 'status', type: 'select', options: [...HOME_MAINTENANCE_STATUSES] },
    { label: 'Provider / contractor', name: 'provider', placeholder: 'Optional' },
    { label: 'Cost', name: 'cost', type: 'number', placeholder: '0.00', step: '0.01', min: '0' },
    { label: 'Details', name: 'notes', type: 'textarea', placeholder: 'Model, filter size, contractor, parts, notes…', wide: true },
  ],
  'Safety record': [
    { label: 'Safety item', name: 'item', required: true, placeholder: 'Upstairs smoke alarm' },
    { label: 'Safety type', name: 'safetyType', type: 'select', options: [...HOME_SAFETY_TYPES] },
    { label: 'Location', name: 'location', required: true, placeholder: 'Second-floor hallway' },
    { label: 'Status', name: 'status', type: 'select', options: [...HOME_SAFETY_STATUSES] },
    { label: 'Last checked', name: 'lastChecked', type: 'date', defaultValue: 'today' },
    { label: 'Next due', name: 'nextDue', type: 'date' },
    { label: 'Model / identifier', name: 'model', placeholder: 'Optional model, serial or expiry detail' },
    { label: 'Notes', name: 'notes', type: 'textarea', placeholder: 'Battery, test result, expiry, replacement notes…', wide: true },
  ],
  Appliance: [
    { label: 'Appliance name', name: 'appliance', required: true, placeholder: 'Kitchen refrigerator' },
    { label: 'Appliance type', name: 'applianceType', type: 'select', options: [...APPLIANCE_TYPES] },
    { label: 'Location', name: 'location', required: true, placeholder: 'Kitchen' },
    { label: 'Status', name: 'status', type: 'select', options: [...APPLIANCE_STATUSES] },
    { label: 'Brand', name: 'brand', placeholder: 'Brand' },
    { label: 'Model', name: 'model', placeholder: 'Model number' },
    { label: 'Serial number', name: 'serial', placeholder: 'Serial number' },
    { label: 'Purchase date', name: 'purchaseDate', type: 'date' },
    { label: 'Warranty ends', name: 'warrantyEnd', type: 'date' },
    { label: 'Retailer', name: 'retailer', placeholder: 'Store / supplier' },
    { label: 'Purchase cost', name: 'cost', type: 'number', placeholder: '0.00', step: '0.01', min: '0' },
    { label: 'Receipt file', name: 'receipt', type: 'file', wide: true },
    { label: 'Manual / document reference', name: 'manual', placeholder: 'URL or local reference', wide: true },
    { label: 'Notes', name: 'notes', type: 'textarea', placeholder: 'Filter sizes, service history, parts, setup notes…', wide: true },
  ],
  Speak: [
