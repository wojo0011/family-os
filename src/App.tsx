...
async function loadWeather(): Promise<Weather> {
  ...
  return {
    ...
    days: (payload.daily?.time ?? []).map((date: string, i: number) => {
      ...
      return {
        date,
        high: Math.round(payload.daily.temperature_2m_max?.[i] ?? 0),
        low: Math.round(payload.daily.temperature_2m_min?.[i] ?? 0),
        rain: Math.round(payload.daily.precipitation_probability_max?.[i] ?? 0),
        icon,
        summary,
      };
    }),
...
function FamilyLensPicker({ value, onChange, compact = false }: { value: PersonId; onChange: (id: PersonId) => void; compact?: boolean }) {
  return <div className={`lens-picker ${compact ? 'compact' : ''}`} role="group" aria-label="Switch family view">
    {people.map(person => <button key={person.id} className={value === person.id ? 'active' : ''} onClick={() => onChange(person.id)} title={`View ${person.name}`} style={{ '--person-color': person.color } as CSSProperties}><span>{person.emoji}</span><b>{person.name}</b></button>)}
  </div>;
}
...
function Today({ events, weather, onAdd, lens }: { events: FamilyEvent[]; weather: Weather | null; onAdd: () => void; lens: PersonId }) {
...
function Calendar({ events, weather, lens, specialDaySet, specialDayEffect }: { events: FamilyEvent[]; weather: Weather | null; lens: PersonId; specialDaySet: SpecialDaySet; specialDayEffect: SpecialDayEffect }) {
  ...
  return <div className="stack">
    ...
    <section className="calendar panel"><div className="weekdays">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => <b key={day}>{day}</b>)}</div><div className="month-grid">{days.map(day => {
      ...
      return <article key={key} className={`${day.getMonth() === anchor.getMonth() ? '' : 'muted'} ${key === dateKey(new Date()) ? 'today' : ''} ${themeClass}`}>
        <header><b>{day.getDate()}</b>{showBirthday ? <span title="Birthday">🎂</span> : showSpecial && special ? <span title={special.label}>{special.icon}</span> : null}</header>
        {w && <div className="tile-weather"><WeatherIcon name={w.icon} alt={w.summary} size="md" /><strong>{w.high}°</strong><small>{w.low}° · 💧{w.rain}%</small></div>}
        <div className="event-list">{eventList.slice(0, 3).map(event => { const owner = activePerson(event.person); return <div key={event.id} style={{ borderLeftColor: owner.color }}><small>{formatTime(event.start)}</small><strong>{event.title}</strong></div>; })}{eventList.length > 3 && <small>+{eventList.length - 3} more</small>}</div>
      </article>;
    })}</div></section>
...
function Settings({ theme, setTheme, lens, setLens, specialDaySet, setSpecialDaySet, specialDayEffect, setSpecialDayEffect }: { theme: string; setTheme: (theme: string) => void; lens: PersonId; setLens: (id: PersonId) => void; specialDaySet: SpecialDaySet; setSpecialDaySet: (value: SpecialDaySet) => void; specialDayEffect: SpecialDayEffect; setSpecialDayEffect: (value: SpecialDayEffect) => void }) {
  ...
  return <div className="stack"><header className="module-hero"><span className="eyebrow">Preferences & privacy</span><h1>Settings</h1><p>Personal appearance and viewing lens can change without changing shared family data.</p></header><section className="settings panel">
    <label>Personal theme<select value={theme} onChange={event => setTheme(event.target.value)}><option value="midnight">Midnight</option><option value="space">Space</option><option value="nature">Nature</option><option value="soft">Soft</option></select></label>
    <div className="setting-block"><div><b>👨‍👩‍👧‍👦 Family lens</b><span>Switch the app between the household view and an individual view.</span></div><FamilyLensPicker value={lens} onChange={setLens} /></div>
    <div className="setting-block"><div><b>🎉 Special-day themes</b><span>Choose which dates are allowed to temporarily decorate calendar tiles.</span></div><div className="theme-set-grid">{specialSets.map(([id, label, description]) => <button key={id} className={specialDaySet === id ? 'selected' : ''} onClick={() => setSpecialDaySet(id)}><strong>{label}</strong><small>{description}</small></button>)}</div></div>
    <label>Special-day effect<select value={specialDayEffect} onChange={event => setSpecialDayEffect(event.target.value as SpecialDayEffect)} disabled={specialDaySet === 'off'}><option value="rich">Rich · background + icon</option><option value="subtle">Subtle · accent + icon</option></select></label>
    <div className="special-preview"><article className={`theme-canada special-${specialDayEffect}`}><b>🇨🇦 Canada Day</b><span>July 1</span></article><article className={`theme-winter special-${specialDayEffect}`}><b>🎄 Christmas</b><span>December 25</span></article><article className={`theme-birthday special-${specialDayEffect}`}><b>🎂 Birthday</b><span>Family milestone</span></article></div>
    <article><b>🔐 Google Calendar</b><span>Private schedules remain protected by Google permissions. A Family Lens never grants access the signed-in account does not already have.</span></article>
    <article><b>🗄 Family Vault</b><span>Health, finance, pet, home and vehicle records are designed for permissioned Google Drive storage.</span></article>
  </section></div>;
}
...
export default function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      <div className="content">{page === 'today' ? <Today events={events} weather={weather} onAdd={() => setCapture(true)} lens={lens} /> : page === 'calendar' ? <Calendar events={events} weather={weather} lens={lens} specialDaySet={specialDaySet} specialDayEffect={specialDayEffect} /> : page === 'settings' ? <Settings theme={theme} setTheme={setTheme} lens={lens} setLens={setLens} specialDaySet={specialDaySet} setSpecialDaySet={setSpecialDaySet} specialDayEffect={specialDayEffect} setSpecialDayEffect={setSpecialDayEffect} /> : <Module page={page} />}</div>
    </main>
